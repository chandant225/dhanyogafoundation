<div>
    <form action="{{ $post->exists ? route('backend.posts.update', $post) : route('backend.posts.store') }}" method="post" class="form" enctype="multipart/form-data">
        @csrf
        @if($post->exists)
        @method('put')
        @endif
        <div class="row">
            ...
            <div class="col-md-3">
                ...
                <x-box class="mt-3">
                    <div class="form-group">
                        <label for="post_category_id" class="required">Post Category</label>
                        <select name="post_category_id" class="custom-select rounded-0 {{ invalid_class('post_category_id') }}">
                            <option value="">Select Category</option>
                            @foreach ($postCategories as $category)
                            <option value="{{ $category->id }}" @if(old('post_category_id', $post->post_category_id) == $category->id) selected @endif>{{ $category->name }}</option>
                            @endforeach
                        </select>
                        <x-invalid-feedback field="post_category_id"></x-invalid-feedback>
                    </div>
                </x-box>
                    ...
                    <div class="text-center">
                        ...
                        <div class="position-relative">
                            ...
                            <label for="postImage" class="position-absolute" style="bottom: 0;
                                        ...
                                        font-size: 0.8em;
                                        cursor: pointer;">
                                <i class="fa fa-camera mr-2"></i> {{ $post->exists ? 'Change' : 'Choose' }} post Image
                            </label>
                        </div>
                        @error('image')
                ...
                <x-box class="mt-3">
                    ...
                    <input type="text" name="tags" id="tags" class="form-control bg-light {{ invalid_class('tags') }}" data-role="tagsinput" value="{{ old('tags', $tags) }}">
                    {{-- <textarea name="tags" id="tags" class="form-control bg-light {{ invalid_class('tags') }}" cols="30" rows="5" data-role="tagsinput">{{ old('tags', $tags) }}</textarea> --}}
                    <x-invalid-feedback field="tags"></x-invalid-feedback>
                </x-box>
            </div>

            <div class="col-md-12">
                <div class="form-group mt-3">
                    <button type="submit" class="btn btn-primary text-md ml-0 px-5">{{ $post->exists ? 'Update' : 'Save' }}</button>
                </div>
            </div>
