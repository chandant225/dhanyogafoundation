    ...
    <div class="container mx-auto py-16 px-4">
        <div class="grid grid-cols-1 sm:grid-cols-12 sm:gap-4 md:gap-5">
            <div class="sm:col-span-7 md:col-span-8">
                ...
                <section class="flex items-center space-x-3 sm:space-x-4 text-indigo-900 font-semibold my-2">
                    <div>
                    ...
                    @hasrole('admin')
                    <a class="hover:underline" href="{{ route('backend.posts.edit', $post) }}">
                        <span class="text-red-500 mr-1">
                            <svg class="inline-flex w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="py-4 flex">
                    <div>
                        <div class="flex space-y-2 space-x-2 flex-wrap items-center">
                            ...
                            <a class="bg-indigo-50 py-2 px-3 group inline-flex items-center space-x-2 rounded-full hover:bg-indigo-500" href="{{ route('posts.index', ['tag' => $tag->name]) }}">
                                <span class="inline-block bg-indigo-500 w-3 h-3 rounded-full group-hover:bg-white"></span>
                                <span class="text-indigo-500 font-semibold text-sm group-hover:text-white">{{ ucfirst($tag->name) }}</span>
                            </a>
                            @endforeach
                        </div>
                    </div>
            ...
            <div class="sm:col-span-7 md:col-span-4">
                <div class="border rounded-xl p-4 sticky top-10">
                    <h3 class="text-xl sm:text-2xl text-indigo-900 font-bold tracking-wide mb-4">{{ __('Recent Posts') }}</h3>
                    <div>
                    ...
                    <div>
                        <h3 class="text-xl sm:text-2xl text-indigo-900 font-bold tracking-wide mb-4">{{ __('Tags') }}</h3>
                        @foreach ($tags as $tag)
                        <a class="bg-indigo-50 py-2 px-3 group inline-flex items-center space-x-2 rounded-full hover:bg-indigo-500 mr-2 mb-3" href="{{ route('posts.index', ['tag' => $tag->name]) }}">
                            <span class="inline-block bg-indigo-500 w-3 h-3 rounded-full group-hover:bg-white"></span>
                            <span class="text-indigo-500 font-semibold text-sm group-hover:text-white">{{ ucfirst($tag->name) }}</span>
                        </a>
                        @endforeach
